...
def load_data(location, filename, modeltype):
    ...
    # appendcsv = np.array([result[x:x+400].flatten()
    #                      for x in range(0, len(result), 400)])
    # np.savetxt(new_file, appendcsv, fmt='%d')
    # x_test = appendcsv.reshape(-1, 400, 4)

    encoded_seq = np.loadtxt('./jobs/'+modeltype+'.txt')
...
def testing_process(x_test, modeltype, location):
    ...
    np.savetxt(os.path.join(location, modeltype +
               'results.txt'), predicted, fmt='%i')


def main(modeltype, filename, location):

    if modeltype == "acceptor_cnnsplice_at.h5":
        x_test = load_data(location, filename, modeltype)
...
def app_init():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument("-t", "--modeltype", type=str, required=True,
                        help="acceptor or donor model and organism name")
    parser.add_argument("-f", "--filename", type=str,
                        required=True, help="name of the input fasta file")
    parser.add_argument("-l", "--location", type=str,
                        required=True, help="directory name to save results")
